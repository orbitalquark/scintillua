...
lex:add_rule('option', token(lexer.PREPROCESSOR, word_match[[
  ...
  IPv6AcceptRouterAdvertisements IPv6DuplicateAddressDetection IPv6HopLimit
  Bridge Bond VLAN MACVLAN VXLAN Tunnel
  -- Address section.
  Address Peer Broadcast Label
  -- Route section.
  Gateway Destination Source Metric Scope PreferredSource
  -- DHCP section.
  UseDNS UseNTP UseMTU SendHostname UseHostname Hostname UseDomains UseRoutes
  UseTimezone CriticalConnection ClientIdentifier VendorClassIdentifier
  ...
  PortRange
  -- Tunnel section.
  Local Remote TOS TTL DiscoverPathMTU IPv6FlowLabel CopyDSCP EncapsulationLimit
  Mode
  -- Peer section.
  Name MACAddress
  -- Tun and Tap section.
...
lex:add_rule('comment', token(lexer.COMMENT,
  lexer.starts_line(lexer.to_eol(S(';#')))))
