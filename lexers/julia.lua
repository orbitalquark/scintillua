-- Copyright 2020 Tobias Frilling. See LICENSE.
-- Julia lexer.
...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  baremodule begin break catch const continue do else elseif end export finally
  for function global if in isa import let local macro module quote return
  struct try using where while
...
local type_builtin_numerical = (
  (P('Abstract') + 'Big') * 'Float' +
  'Float' * (P('16') + '32' + '64') +
  P('U')^-1 * 'Int' * (P('8') + '16' + '32' + '64' + '128')^-1 +
  P('Abstract')^-1 * 'Irrational'
) * -lexer.alnum + word_match[[
  Number Complex Real Integer Bool Signed Unsigned Rational
]]
...
local type_builtin_array = (
  (P('Abstract') + 'Bit' + 'Dense' + 'PermutedDims' + 'Sub')^-1 *
  word_match('Array Vector Matrix VecOrMat') +
  (P('Abstract') + 'Sym' + (P('Unit')^-1 * (P('Lower') + 'Upper')))^-1 *
  'Triangular'
) * -lexer.alnum + word_match[[
  Adjoint Bidiagonal Diagonal Hermitian LQPackedQ Symmetric Transpose
  UpperHessenberg
]]
lex:add_rule('type', token(lexer.TYPE, type_para + type_annotated +
  type_subtyping + type_struct + type_builtin_numerical + type_builtin_range +
  type_builtin_array))
...
local float_hex = float_hex_coeff * float_hex_expon^-1 +
  hexadecimal * float_hex_expon
