...
lex:add_rule('function', token(lexer.KEYWORD, word_match[[fun new be]]) *
  ...
  word_match[[
    ...
    eq_unsafe ne_unsafe lt_unsafe le_unsafe ge_unsafe gt_unsafe neg neg_unsafe
    op_not
    has_next next
...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  actor as be break class compile_error compile_intrinsic continue consume do
  else elseif embed end error for fun if ifdef iftype in interface is isnt
  lambda let match new object primitive recover repeat return struct then this
  trait try type until use var where while with
]]))
...
lex:add_rule('identifier', token(lexer.IDENTIFIER, P('_')^-1 * lexer.lower *
  id_suffix))
...
local function num(digit)
  return digit * (digit^0 * '_')^0 * digit^1 + digit
end
...
lex:add_rule('number', token(lexer.NUMBER, '0x' * num(lexer.xdigit) +
  P('0b') * num(S('01')) + int * frac^-1 * exp^-1))
...
lex:add_rule('punctuation', token(lexer.OPERATOR, P('=>') + '.>' + '<:' + '->' +
  S('=.,:;()[]{}!?~^&|_@')))

-- Qualifiers.
lex:add_rule('qualifier', token(lexer.LABEL, '#' * word_match[[
