...
local enum_list = P('(')^-1 *
  (lexer.digit^1 + S('ivxlcmIVXLCM')^1 + lexer.alnum + '#') * S('.)')
...
local list = #(lexer.space^0 * (S('*+-:/') + enum_list)) *
  starts_line(token('list', lexer.space^0 *
...
local sphinx_directive = token('sphinx_directive', prefix *
  sphinx_directive_type * '::' * lexer.space)
...
lex:add_rule('substitution', substitution)
...
local substitution_ref = token('substitution', lexer.range('|', true) *
  ('_' * P('_')^-1)^-1)
local link = token('link', lexer.alpha * (lexer.alnum + S('-.'))^1 * ':' *
  (lexer.alnum + S('/.+-%@'))^1)
local inline_markup = (strong + em + inline_literal + link_ref + interpreted +
  footnote_ref + citation_ref + substitution_ref + link) * -lexer.alnum
lex:add_rule('inline_markup', inline_markup)

-- Other.
lex:add_rule('non_space', token(lexer.DEFAULT,
                          lexer.alnum * (lexer.any - lexer.space)^0))
lex:add_rule('escape', token(lexer.DEFAULT, '\\' * lexer.any))

lex:add_style('list', lexer.STYLE_TYPE)
lex:add_style('literal_block', lexer.STYLE_EMBEDDED .. ',eolfilled')
...
lex:add_style('sphinx_directive', lexer.STYLE_KEYWORD .. ',bold')
lex:add_style('unknown_directive', lexer.STYLE_KEYWORD .. ',italics')
lex:add_style('code_block', lexer.STYLE_EMBEDDED .. ',eolfilled')
...
lex:add_style('footnote', 'underlined')
lex:add_style('citation', 'underlined')
