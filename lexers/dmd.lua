...
-- Class names.
lex:add_rule('class', token(lexer.TYPE, P('class') + 'struct') * ws^-1 *
  token(lexer.CLASS, lexer.word))

-- Versions.
local version = word_match[[
  AArch64 AIX all Alpha Alpha_HardFloat Alpha_SoftFloat Android ARM
  ARM_HardFloat ARM_SoftFloat ARM_SoftFP ARM_Thumb assert BigEndian BSD Cygwin
  D_Coverage D_Ddoc D_HardFloat DigitalMars D_InlineAsm_X86 D_InlineAsm_X86_64
  D_LP64 D_NoBoundsChecks D_PIC DragonFlyBSD D_SIMD D_SoftFloat D_Version2 D_X32
  FreeBSD GNU Haiku HPPA HPPA64 Hurd IA64 LDC linux LittleEndian MIPS32 MIPS64
  MIPS_EABI MIPS_HardFloat MIPS_N32 MIPS_N64 MIPS_O32 MIPS_O64 MIPS_SoftFloat
  NetBSD none OpenBSD OSX Posix PPC PPC64 PPC_HardFloat PPC_SoftFloat S390 S390X
  SDC SH SH64 SkyOS Solaris SPARC SPARC64 SPARC_HardFloat SPARC_SoftFloat
  SPARC_V8Plus SysV3 SysV4 unittest Win32 Win64 Windows X86 X86_64
]]
lex:add_rule('version', token(lexer.KEYWORD, 'version') * ws^-1 *
  token(lexer.OPERATOR, '(') * ws^-1 * token('versions', version))
...
lex:add_rule('trait', token(lexer.KEYWORD, '__traits') * ws^-1 *
  token(lexer.OPERATOR, '(') * ws^-1 * token('traits', trait))
...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  abstract align asm assert auto body break case cast catch const continue debug
  default delete deprecated do else extern export false final finally for
  foreach foreach_reverse goto if import immutable in inout invariant is lazy
  macro mixin new nothrow null out override pragma private protected public pure
  ref return scope shared static super switch synchronized this throwtrue try
  typeid typeof unittest version virtual volatile while with __gshared __thread
  __traits __vector __parameters
]]))

-- Types.
local type = token(lexer.TYPE, word_match[[
  alias bool byte cdouble cent cfloat char class creal dchar delegate double
  enum float function idouble ifloat int interface ireal long module package
  ptrdiff_t real short size_t struct template typedef ubyte ucent uint ulong
  union ushort void wchar string wstring dstring hash_t equals_t
]])
...
-- Properties.
lex:add_rule('property', lpeg.B(lexer.alnum + ')') *
  token(lexer.OPERATOR, '.') * token(lexer.VARIABLE, word_match[[
    alignof dig dup epsilon idup im init infinity keys length mangleof mant_dig
    max max_10_exp max_exp min min_normal min_10_exp min_exp nan offsetof ptr
