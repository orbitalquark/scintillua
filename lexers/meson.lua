...
local method_names = word_match[[
  ...
  is_even is_odd
  -- string --
  contains endswith format join split startswith substring strip to_int to_lower
  to_upper underscorify version_compare
  -- meson object --
  ...
  can_run_host_binaries has_exe_wrapper install_dependency_manifest
  is_cross_build is_subproject is_unity override_find_program
  override_dependency project_version project_license project_name version
  -- *_machine object --
  cpu_family cpu system endian
  -- compiler object --
  alignment cmd_array compiles compute_int find_library first_supported_argument
  first_supported_link_argument get_define get_id get_argument_syntax
  get_linker_id get_supported_arguments get_supported_link_arguments
  has_argument has_link_argument has_function check_header has_header
  has_header_symbol has_member has_members has_multi_arguments
  has_multi_link_arguments has_type links run symbols_have_underscore_prefix
  sizeof version has_function_attribute get_supported_function_attributes
  -- build target object --
...
local func_names = word_match[[
  ...
  add_project_arguments add_project_link_arguments add_test_setup alias_targ
  assert benchmark both_libraries build_target configuration_data configure_file
  custom_target declare_dependency dependency disabler error environment
  executable find_library find_program files generator get_option get_variable
  import include_directories install_data install_headers install_man
  install_subdir is_disabler is_variable jar join_paths library message warning
  summary project run_command run_targ set_variable shared_library shared_module
...
-- Builtin objects.
-- https://mesonbuild.com/Reference-manual.html#builtin-objects
lex:add_rule('object', token('object', word_match[[
