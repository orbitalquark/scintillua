...
-- Structures.
local struct_dec = token(lexer.KEYWORD, P('structure')) * ws *
  token(lexer.CLASS, aid) * ws * token(lexer.OPERATOR, P('=')) * ws
lex:add_rule('struct_new', struct_dec * token(lexer.KEYWORD, P('struct')))
...
lex:add_rule('keyword', token(lexer.KEYWORD, word_match[[
  abstype and andalso as case do datatype else end exception fn fun handle if in
  infix infixr let local nonfix of op orelse raise rec then type val with
  withtype while
...
lex:add_rule('type', token(lexer.TYPE, word_match[[
  int real word bool char string unit array exn list option order ref substring
  vector
]]))
...
lex:add_rule('function', token(lexer.FUNCTION, word_match[[
  app before ceil chr concat exnMessage exnName explode floor foldl foldr getOpt
  hd ignore implode isSome length map not null ord print real rev round size str
  substring tl trunc valOf vector o abs mod div
]]))
