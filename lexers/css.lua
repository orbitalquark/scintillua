...
local selector = '*' * l.space + (class_id_selector + identifier + '*') * attribute^-1
local pseudoclass = word_match {
  'first-letter', 'first-line', 'link', 'active', 'visited',
  'first-child', 'focus', 'hover', 'lang', 'before', 'after',
  'left', 'right', 'first'
}
selector = token('css_selector', selector * (l.space * selector)^0) *
...
local url = token('css_url', 'url' * ('(' * (sq_str + dq_str) * ')' + l.delimited_range('()', '\\', true)))

-- declaration block
local block_default_char = token('default', (l.any - '}')^0) -- used to keep block colored properly
